...
export default function ProfitLossReport() {
  ...
  const fetchReportPdf = async () => {
    try {
      setLoading(true);
      // URL for direct PDF preview with dynamic language
      const currentLang = i18n.language || 'id';
      const response = await apiClient.get(`/reports/profit-loss/pdf?start_date=${startDate}&end_date=${endDate}&lang=${currentLang}`, {
        responseType: 'blob'
      });
  ...
  useEffect(() => {
    fetchReportPdf();
  }, [i18n.language]); // Trigger on language change
  ...
  return (
    <div className="w-full">
      ...
      {loading && !pdfUrl ? (
        ...
        <A4Paper title={t('menu_profit_loss')} pdfUrl={pdfUrl} onRefresh={fetchReportPdf} />
      ) : (
