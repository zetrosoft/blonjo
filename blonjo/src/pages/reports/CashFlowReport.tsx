...
export default function CashFlowReport() {
  ...
  const fetchReportPdf = async () => {
    try {
      ...
      const response = await apiClient.get(`/reports/cash-flow/pdf?start_date=${startDate}&end_date=${endDate}&lang=${currentLang}`, {
        responseType: 'blob'
      });
  ...
  return (
    <div className="w-full">
      ...
      {loading && !pdfUrl ? (
        ...
        <A4Paper title={t('menu_cash_flow')} pdfUrl={pdfUrl} onRefresh={fetchReportPdf} />
      ) : (
