...
export default function EquityChangesReport() {
  ...
  const fetchReportPdf = async () => {
    try {
      ...
      const response = await apiClient.get(`/reports/equity-changes/pdf?start_date=${startDate}&end_date=${endDate}&lang=${currentLang}`, {
        responseType: 'blob'
      });
  ...
  return (
    <div className="w-full">
      ...
      {loading && !pdfUrl ? (
        ...
        <A4Paper title={t('menu_equity_changes')} pdfUrl={pdfUrl} onRefresh={fetchReportPdf} />
      ) : (
